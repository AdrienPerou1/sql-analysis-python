...
insert into _academie (code_academie, lib_academie)
  ...
  from _temp_fr_en_etablissements_ep
  where (code_academie, libelle_academie) not in (select code_academie, lib_academie from _academie) and (code_academie, libelle_academie) is not null;
...
insert into _quartier_prioritaire (code_quartier_prioritaire, nom_quartier_prioritaire)
  ...
  from _temp_fr_en_etablissements_ep
  where (qp_a_proximite, nom_du_qp) not in (select code_quartier_prioritaire, nom_quartier_prioritaire from _quartier_prioritaire) and
  (qp_a_proximite, nom_du_qp) is not null and qp_a_proximite_o_n!='Hors QP';
...
insert into _annee (annee_scolaire)
    ...
    from _temp_fr_en_college_effectif_niveau_sexe_lv
  where (rentree_scolaire) not in (select annee_scolaire from _annee);

-- Type

delete from _type;
insert into _type (libelle_nature, code_nature)
  select distinct libelle_nature, code_nature
  from _temp_fr_en_etablissements_ep
  where (libelle_nature,code_nature) not in (select libelle_nature,code_nature from _type) and (libelle_nature,code_nature) is not null;

-- Etablissement

--Il manque des données dans commune

INSERT INTO _commune (code_insee_de_la_commune, nom_de_la_commune, code_du_departement)
SELECT DISTINCT code_commune, nom_commune, code_departement
FROM _temp_fr_en_etablissements_ep
WHERE (code_commune, nom_commune, code_departement) NOT IN (SELECT code_insee_de_la_commune, nom_de_la_commune, code_du_departement FROM _commune);
...
-- caracteristiques_tout_etablissement
...
INSERT INTO _caracteristiques_tout_etablissement(uai, annee_scolaire, effectifs)
  SELECT DISTINCT numero_college, rentree_scolaire, nombre_eleves_total
  FROM _temp_fr_en_college_effectif_niveau_sexe_lv
  WHERE (numero_college, rentree_scolaire, nombre_eleves_total) NOT IN
    (SELECT uai, annee_scolaire, effectifs FROM _caracteristiques_tout_etablissement)
  AND (numero_college, rentree_scolaire, nombre_eleves_total) IS NOT NULL
  AND numero_college in (select uai from _etablissement);
